...
Build outputs are not kept here. That covers the packaged flasher installer
(`AutoPost_Pi_Setup.exe`, ~186 MB) and the Electron/Chromium runtime it ships with —
`vulkan-1.dll`, `vk_swiftshader.dll` and its ICD manifest, `libEGL.dll`, `d3dcompiler_47.dll`,
`ffmpeg.dll`, `chrome_*.pak`, `icudtl.dat`. All of it is produced by electron-builder from
`connector/flasher/`, so it is regenerated by a build rather than versioned; `.gitignore`
excludes it. The signed installer belongs on a release page or file share, not in the repo.
